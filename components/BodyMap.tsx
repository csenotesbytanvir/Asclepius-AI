...
export const BodyMap: React.FC<BodyMapProps> = ({ onSelect, selectedPart }) => {
  ...
  return (
    <div className="flex flex-col items-center w-full">
      {/* View Toggle */}
      <div className="bg-surfaceHighlight/30 backdrop-blur-sm p-1.5 rounded-2xl mb-8 shadow-lg border border-border w-full max-w-sm flex">
         <button 
           type="button"
             ...
             <button
                ...
                className={clsx(
                    ...
                    selectedPart === part.id 
                      ? "border-primary bg-gradient-to-br from-primary to-accent shadow-glow -translate-y-2 scale-105" 
                      : "border-border bg-surface hover:bg-surfaceHighlight hover:border-primary/30 hover:-translate-y-2 hover:shadow-xl"
                )}
             >
                {/* Background Glow Effect on Hover (Only visible when NOT selected) */}
                <div className={clsx(
                    "absolute inset-0 bg-gradient-to-tr from-primary/10 via-accent/5 to-transparent opacity-0 transition-opacity duration-500",
                    selectedPart !== part.id ? "group-hover:opacity-100" : "opacity-0"
                )}></div>
                ...
                <div className="text-center relative z-10 p-4">
                    <span className={clsx(
                        ...
                        selectedPart === part.id ? "text-white drop-shadow-md" : "text-textSecondary group-hover:text-primary"
                    )}>
                        {part.label}
                    </span>
                    <span className={clsx(
                         "text-[10px] uppercase tracking-widest mt-2 block transition-all duration-300",
                         selectedPart === part.id ? "text-white/80 font-bold" : "text-textSecondary/50 opacity-0 group-hover:opacity-100 group-hover:translate-y-0 translate-y-2"
                    )}>
                        Select Region
                    </span>
                </div>
